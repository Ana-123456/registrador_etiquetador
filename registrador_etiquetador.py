...
RUTA_CREDENCIALES = '/data/data/com.termux/files/home/botstel/credentials/credentials.json'

# Cliente de Telethon (CUENTA PERSONAL)
client = TelegramClient('bot_fusionado', api_id, api_hash)

# ==============================
# CONEXIÓN A GOOGLE SHEETS (una sola vez)
# ==============================
alcance = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive", "https://www.googleapis.com/auth/spreadsheets"]
...
hoja_registro = documento.worksheet("Usuarios")  # Solo se abre una vez aquí
...
async def reporte_etiquetador():
    try:
        while True:
            ...
            horarios = [
                ahora.replace(hour=12, minute=1, second=0, microsecond=0),
                ahora.replace(hour=19, minute=1, second=0, microsecond=0)
            ]

            siguiente = next((h for h in horarios if ahora < h), (ahora + timedelta(days=1)).replace(hour=12, minute=1))
            await asyncio.sleep((siguiente - ahora).total_seconds())
...
async def main():
    ...
    # Tareas paralelas para los reportes automáticos
